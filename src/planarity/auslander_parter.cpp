...
Embedding merge_biconnected_components(
    ...
    for (size_t i = 0; i < biconnected_components.get_components().size();
         ++i) {
...
Embedding base_case_component(
    const UndirectedGraph& component,
    const Cycle& cycle) {
    Embedding embedding(component);
    for (int node_id : component.get_nodes_ids()) {
        if (component.get_degree_of_node(node_id) == 2) {
            for (int neighbor_id : component.get_neighbors_of_node(node_id))
                embedding.add_edge(node_id, neighbor_id);
            continue;
        }
...
std::vector<bool> are_embeddings_inside_clockwise_cycle(
    ...
    for (size_t i = 0; i < segments.size(); ++i) {
        ...
        is_inside[i] =
            embedding.get_adjacency_list(attachment_id).next_element(next) !=
            prev;
    }
    return is_inside;
}
...
void add_middle_edges(
    ...
    for (size_t i = 1; i < embedding.get_adjacency_list(cycle_node_id).size();
         ++i) {
        current =
            embedding.get_adjacency_list(cycle_node_id).next_element(current);
...
std::optional<Embedding> embed_biconnected_component(
    ...
    const UndirectedGraph interlacement_graph =
    ...
    return merge_segments_embeddings(
        component, cycle, embeddings, segments, is_segment_inside.value());
}
...
std::optional<Embedding> embed_graph(const UndirectedGraph& graph) {
    ...
    if (graph.get_number_of_edges() / 2 > 3 * graph.size() - 6)
        return std::nullopt;
    const BiconnectedComponents bic_comps =
        compute_biconnected_components(graph);
    std::cout << "daje\n";
    std::vector<Embedding> embeddings;
