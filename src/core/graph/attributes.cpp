...
// node color
void Attributes::set_node_color(size_t node_id, Color color) {
...
Color Attributes::get_node_color(size_t node_id) const {
    DOMUS_ASSERT(
        m_nodes_color->at(node_id).has_value(),
...
void Attributes::change_node_color(size_t node_id, Color color) {
    DOMUS_ASSERT(
        m_nodes_color->at(node_id).has_value(),
...
// position
void Attributes::set_position(size_t node_id, int x, int y) {
...
void Attributes::change_position(size_t node_id, int x, int y) {
    DOMUS_ASSERT(
        m_nodes_position->at(node_id).has_value(),
...
void Attributes::change_position_x(size_t node_id, int x) {
    DOMUS_ASSERT(
        m_nodes_position->at(node_id).has_value(),
        "GraphAttributes::change_position_x: node does not have a position"
    );
    m_nodes_position->at(node_id)->x = x;
}

void Attributes::change_position_y(size_t node_id, int y) {
    DOMUS_ASSERT(
        m_nodes_position->at(node_id).has_value(),
        "GraphAttributes::change_position_y: node does not have a position"
    );
    m_nodes_position->at(node_id)->y = y;
}

int Attributes::get_position_x(size_t node_id) const {
    DOMUS_ASSERT(
        m_nodes_position->at(node_id).has_value(),
        "GraphAttributes::get_position_x: node does not have a position"
    );
    return m_nodes_position->at(node_id)->x;
}

int Attributes::get_position_y(size_t node_id) const {
    DOMUS_ASSERT(
        m_nodes_position->at(node_id).has_value(),
        "GraphAttributes::get_position_y: node does not have a position"
    );
    return m_nodes_position->at(node_id)->y;
}
